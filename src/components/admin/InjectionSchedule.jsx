...
const InjectionSchedule = () => {
  ...
  const handleInstantSync = async (ruleId, category) => {
    console.log(`--- [INSTANT TRIGGER] Pipeline: ${category} (ID: ${ruleId}) ---`);
    setSyncingId(ruleId);
    try {
      console.log(`[UI] Dispatching instant ingestion for ${category}...`);
      const result = await runIngestionForActiveSchedules({ onlyScheduleId: ruleId, force: true });
      
      if (result.success) {
        console.log(`[UI] Sync successful. Articles processed: ${result.processedCount}`);
        alert(`Success! Generated ${result.processedCount} new articles for ${category}.`);
        fetchRules(); // Refresh list to see updated counts
      } else {
        console.warn(`[UI] Sync skipped or failed: ${result.reason || result.error}`);
        alert(`Notice: ${result.reason || result.error || "No new articles generated."}`);
      }
    } catch (err) {
      console.error(`[UI] Fatal error during instant sync:`, err);
      alert("Instant sync failed. Check console for details.");
    } finally {
      setSyncingId(null);
      console.log(`--- [INSTANT TRIGGER FINISHED] ---`);
    }
  };
            ...
            <div className="flex justify-between items-start mb-6 md:mb-10">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  onClick={() => handleInstantSync(rule._id, rule.category)}
                  className={`p-2 md:p-3 rounded-lg md:rounded-xl bg-accent/10 text-accent hover:bg-accent hover:text-white transition-all active:scale-95 shrink-0 ${syncingId === rule._id ? 'animate-spin' : ''}`}
                  title="Instant Sync"
                >
                  <RefreshCw size={16} />
                </button>
      ...
      const fetchSilos = async () => {
        try {
          ...
          if (res.length > 0 && !formData.category) {
            // IMPORTANT: schedules.category is treated as a human category label/search term,
            // so store the category NAME (not slug).
            setFormData(prev => ({ ...prev, category: res[0].name }));
          }
        } catch (err) {
            ...
            <div className="space-y-4">
              ...
              <div className="bg-surface p-6 rounded-2xl border border-border shadow-inner relative overflow-hidden">
                ...
                <input
                  ...
                  onChange={e => setFormData({ ...formData, articlesPerDay: parseInt(e.target.value, 10) })}
