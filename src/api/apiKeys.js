...
export async function getApiKey(provider) {
  ...
  if (error) {
    console.error("Error fetching API keys for rotation:", error);
    return null;
  }
  ...
  for (const row of data || []) {
    ...
    // Treat total_quota = 0 as unlimited, otherwise check remaining quota.
    // Also rotate if usage is >= 100%
    if (total === 0 || used < total) {
      // Async update usage without blocking the request
