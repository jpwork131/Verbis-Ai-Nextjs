...
const SnippetCard = ({ article }) => {
  ...
  return (
    <div className="flex gap-3 pb-[12px] border-b border-[var(--color-border)] last:border-0 group cursor-pointer mt-3">
...
export default function EntrackrHomepage({ marketNewsWidget }) {
  ...
  return (
    <div className="entrackr-theme min-h-screen bg-[var(--color-bg)]">
      ...
      <div className="max-w-[1200px] mx-auto mt-4 px-4 text-decoration-none">
        <Link href={activeBanner?.cta_link || "#"} className="block w-full h-[80px] rounded-lg bg-gradient-to-r from-[#1E293B] to-[var(--color-accent)] flex items-center justify-between px-8 text-white overflow-hidden relative">
          ...
          <div className="hidden lg:block h-full py-4 z-10">
            <div className="font-display text-[24px] font-bold opacity-30">Verbis Ai</div>
          </div>
      ...
      {/* 10. Footer */}
      <footer className="w-full bg-[var(--color-footer-bg)] pt-12 text-white">
        <div className="max-w-[1200px] mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">

            {/* Col 1 - Brand */}
            <div>
              <div className="font-display text-[24px] font-bold tracking-tight mb-4">
                <span className="text-[var(--color-accent)]">Verbis</span>
                <span className="text-white">Ai</span>
              </div>
              <p className="text-[var(--color-text-muted)] text-[13px] font-body leading-relaxed max-w-[250px]">
                Verbis Ai Is A New Age Media Platform For Entrepreneurs, Startups And Technology Enthusiast.
              </p>
            </div>

            {/* Col 2 - Newsletter */}
            <div>
              <h4 className="font-body text-[12px] font-bold uppercase tracking-wider mb-4">
              ...
              <form onSubmit={handleSubscribe} className="space-y-3">
                <input
                  ...
                  className="w-full bg-[#1E293B] border border-[#334155] rounded px-4 py-2 text-[13px] focus:outline-none focus:border-[var(--color-accent)]"
                />
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-[#1E293B] border border-[#334155] rounded px-4 py-2 text-[13px] focus:outline-none focus:border-[var(--color-accent)]"
                    required
                  />
                  <button
                    type="submit"
                    className="bg-[var(--color-accent)] text-white px-4 py-2 rounded text-[12px] font-bold whitespace-nowrap hover:bg-[var(--color-accent-dark)] transition-colors"
                  >
                    Subscribe Now
                  </button>
                </div>
            ...
            <div className="md:text-right">
              <h4 className="font-body text-[12px] font-bold uppercase tracking-wider mb-4 md:hidden">SOCIAL</h4>
              <div className="flex md:justify-end gap-3">
                {[
                  ...
                  <div
                    key={i}
                    className="w-[40px] h-[40px] rounded-full bg-[#1E293B] border border-[#334155] flex items-center justify-center text-white transition-all cursor-pointer hover:scale-110"
                    onMouseEnter={(e) => e.currentTarget.style.backgroundColor = color}
          ...
          <div className="border-t border-[#1E293B] py-6 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 text-[12px] text-[var(--color-text-muted)] font-body">
              <span className="text-white font-bold">QUICK LINKS</span>
              {['About Us', 'Terms Of Use', 'Privacy Policy', 'Disclaimer', 'Contact Us'].map((link, i) => (
                <div key={link} className="flex items-center gap-4">
                  <Link href={`/${link.toLowerCase().replace(/ /g, '-')}`} className="hover:text-white transition-colors">{link}</Link>
                  {i < 4 && <span>·</span>}
                </div>
              ))}
            </div>
            <div className="text-[12px] text-[#64748B] font-body">
              © All Rights Reserved Bareback Media Pvt. Ltd.
            </div>
          </div>
        </div>
      </footer>
