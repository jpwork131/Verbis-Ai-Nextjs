...
export async function subscribeToNewsletter(email) {
  try {
    // 1. Existing Supabase subscription
    const { error } = await supabase
      .from('subscribers')
      .insert({ email });

    // 2. Google Sheets submission
    // We call our internal API route which has the server-side credentials
    fetch('/api/submit', {
      method: 'POST',
    ...
    }).catch(err => console.error("Google Sheets Newsletter error:", err));
