...
export const createArticleAI = async (articleData) => {
  ...
  if (provider === 'disabled') {
    ...
    const [openrouterKey, gnewsKey, geminiKey, groqKey] = await Promise.all([
      getApiKey("openrouter"),
      getApiKey("gnews"),
      getApiKey("google_gemini"),
      getApiKey("groq")
...
JSON STRUCTURE:
{
  ...
  "seoKeywords": ["key1", "key2", "key3", "key4", "key5"]
    ...
    const executeRewrite = async (engine, key) => {
      if (!key) throw new Error(`Missing ${engine} API key.`);
      
      if (engine === 'google_gemini') {
        const llmRes = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${key}`, {
          method: "POST",
        ...
        if (!llmRes.ok) {
          ...
          throw new Error(`Gemini failed: ${llmRes.status} ${errText}`);
        }

        const llmData = await llmRes.json();
        return JSON.parse(llmData.candidates[0].content.parts[0].text);
      } 
      ...
      if (engine === 'groq') {
        ...
        if (!llmRes.ok) {
          const errText = await llmRes.text();
          throw new Error(`Groq failed: ${llmRes.status} ${errText}`);
        }
      ...
      if (!llmRes.ok) {
        const errText = await llmRes.text();
        throw new Error(`OpenRouter failed: ${llmRes.status} ${errText}`);
      }
    ...
    // Failover sequence implementation
    let attempts = [];
    if (provider === 'random') {
      attempts = [
        { name: 'openrouter', key: openrouterKey },
        { name: 'google_gemini', key: geminiKey },
        { name: 'groq', key: groqKey }
      ].sort(() => Math.random() - 0.5);
    } else {
      // If a specific provider is selected, we still want to failover to others if it fails
      const primaryKey = provider === 'openrouter' ? openrouterKey : (provider === 'google_gemini' ? geminiKey : groqKey);
      attempts = [{ name: provider, key: primaryKey }];
      
      // Add others as failover
      const others = [
        { name: 'openrouter', key: openrouterKey },
        { name: 'google_gemini', key: geminiKey },
        { name: 'groq', key: groqKey }
      ].filter(a => a.name !== provider).sort(() => Math.random() - 0.5);
      
      attempts = [...attempts, ...others];
    }

    emitLog(`[AI GEN] Starting rewrite sequence. Strategy: ${provider.toUpperCase()}`);

    let success = false;
    for (const attempt of attempts) {
      if (!attempt.key) {
        emitLog(`[AI GEN] Skipping ${attempt.name}: No API key found.`);
        continue;
      }

      try {
        emitLog(`[AI GEN] Attempting rewrite with ${attempt.name.toUpperCase()}...`);
        parsed = await executeRewrite(attempt.name, attempt.key);
        modelUsed = attempt.name;
        success = true;
        emitLog(`[AI GEN] Rewrite successful with ${attempt.name.toUpperCase()}!`);
        break;
      } catch (err) {
        emitLog(`[AI GEN] Error with ${attempt.name}: ${err.message}. Trying next...`, true);
      }
    }

    if (!success) {
      emitLog(`[AI GEN] CRITICAL: All rewrite models failed! Falling back to raw content.`, true);
    } else {
      emitLog(`[AI GEN] Rewrite Sequence Finalized. Target Title: "${parsed.title}"`);
    }
  }
...
const mapArticle = (row) => {
  ...
  return {
    ...
    categoryColor: row.categories?.badge_color || row.categoryColor || '#3b82f6'
  };
};
...
export const getArticles = async (page = 1, limit = 10) => {
  ...
  const { data, error, count } = await supabase
    .from('articles')
    .select('*, categories(badge_color)', { count: 'exact' })
