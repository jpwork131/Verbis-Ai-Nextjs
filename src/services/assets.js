...
export const assetApi = {
  ...
  updateAssets: async (frontendData, files = {}) => {
    ...
    if (files.logo) {
      try {
        updates.logo = await uploadToCloudinary(files.logo, "startej_branding");
      } catch (err) {
    ...
    if (files.fallbackBanner) {
      try {
        updates.fallbackBannerUrl = await uploadToCloudinary(files.fallbackBanner, "startej_branding");
      } catch (err) {
